	...
	def dispatch(self, msg):
		if not hasattr(msg, 'channel'):
			...
			if msg.type == 'note_on' and msg.velocity > 0:
				if all(self.active):
					...
					for j in range(self.polyphony):
						if self.note[j] in [top, bottom]:
							...
							i = j
					self.outport.send(mido.Message('note_off', note=self.note[i], channel=i+1))
				...
				if msg.channel in self.mapping and msg.note == self.note[self.mapping.index(msg.channel)]:
