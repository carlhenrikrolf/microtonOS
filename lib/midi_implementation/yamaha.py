...
class RefaceCP:
    ...
    sostenuto_control = 66
    soft_pedal_control = 67
    ...
    def parameter_change(self, address, data):
        if type(data) is list:
            return mido.Message(
                "sysex", data=[*self.prefix, self.model_id, *address, *data]
            )
        else:
            return mido.Message(
                "sysex", data=[*self.prefix, self.model_id, *address, data]
    ...
    def chorus(self, msg):
        if msg.is_cc(self.chorus_phaser_control):
            if msg.value == self.up:
                ...
                return (
                    True,
                    self.chorus_phaser_depth_value,
                    self.chorus_phaser_speed_value,
                )
    ...
    def digital_delay(self, msg):
        if msg.is_cc(self.digital_analog_control):
            if msg.value == self.up:
                ...
                return (
                    True,
                    self.digital_analog_depth_value,
                    self.digital_analog_time_value,
                )
    ...
    def sustain(self, msg=None):
        if msg.is_cc(self.sustain_control):
            self.sustain_value = msg.value
            return self.sustain_value
        return None

    def modwheel(self, value=0, channel=0):
        return mido.Message(
            "control_change",
            control=self.modwheel_control,
            value=value,
            channel=channel,
        )

    def volume(self, value=0, channel=0):
        return mido.Message(
            "control_change", control=self.volume_control, value=value, channel=channel
        )

    def expression(self, value=0, channel=0):
        return mido.Message(
            "control_change",
            control=self.expression_control,
            value=value,
            channel=channel,
        )

    def sostenuto(self, value=0, channel=0):
        return mido.Message(
            "control_change",
            control=self.sostenuto_control,
            value=value,
            channel=channel,
        )

    def soft_pedal(self, value=0, channel=0):
        return mido.Message(
            "control_change",
            control=self.soft_pedal_control,
            value=value,
            channel=channel,
        )
