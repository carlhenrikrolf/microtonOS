...
class Isomorphic:
    null_note = 127
    def __init__(self):
        self.mapping = xq.default_map
        self.coloring = [Color("black")] * 128
    ...
    def send(self, outport, layout=None, coloring=None):
        ...
        for key, note in enumerate(self.mapping):
            if note == self.null_note:
                xq.send(outport, xq.sysex(xq.color_key, key, xq.led("white")))
            else:
                xq.send(
                    outport, xq.sysex(xq.color_key, key, xq.led(self.coloring[note]))
                )

    def highlight(self, outport, msg):
        if msg.type in ["note_on", "note_off"]:
            if msg.note != self.null_note:
                for key, note in enumerate(self.mapping):
                    if note == msg.note:
                        if msg.type == "note_on" and msg.velocity > 0:
                            xq.send(
                                outport, xq.sysex(xq.color_key, key, xq.led("white"))
                            )
                        else:
                            color = self.coloring[note]
                            xq.send(outport, xq.sysex(xq.color_key, key, xq.led(color)))
